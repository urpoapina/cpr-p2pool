...
#            subsidy = self.node.net.PARENT.SUBSIDY_FUNC(self.node.pow_bits.target)
    ...
    def preprocess_request(self, user):
        user, script, desired_share_target, desired_pseudoshare_target = self.get_user_details(user)
        return script, desired_share_target, desired_pseudoshare_target
    
    def get_work(self, _script, desired_share_target, desired_pseudoshare_target):
        ...
        if len(self.recent_shares_ts_work) > 1:
            hash_rate = sum(work for ts, work in self.recent_shares_ts_work[1:])//(self.recent_shares_ts_work[-1][0] - self.recent_shares_ts_work[0][0])
            if hash_rate:
	        target = min(target, int(2**256/(hash_rate * 2)))
