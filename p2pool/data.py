...
DONATION_SCRIPT = '41043f0cf60cee50710cbaa651bb566cabdfffe3b46d6f943d7a5bf75ec6318357ab0443c6eaa28e13f19b9113d7ae88c7717b8740bc1140e459ae6c0bda0bdedb6dac'.decode('hex')
...
def get_user_stale_props(tracker, share_hash, lookbehind):
    res = {}
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        stale, total = res.get(share.share_data['script'], (0, 0))
        total += 1
        if share.share_data['stale_info'] is not None:
            stale += 1
            total += 1
        res[share.share_data['script']] = stale, total
    return dict((script, stale/total) for script, (stale, total) in res.iteritems())
